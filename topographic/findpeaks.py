    ...
    if ind[0] == 0:
        if dy[0] == 0:         # There is a plateau at the start
            ...
            if dy[non_zero_ind[0][0]] > 0:
                # The plateau at the start is a valley,
    ...
    if ind[-1] == np.size(y):
        if dy[-1] == 0:
            ...
            if dy[non_zero_ind[0][-1]] < 0:
                # The plateau at the end is a valley, so remove it from the
                # list
                ind = ind[0:-2]
...
def findvalleys(y):
    ...
    :return: :class:`numpy.ndarray` the index values of the valleys in the line
    """
