...
def rasterize(input, output):
	...
	print(cols, rows)
	target_ds = gdal.GetDriverByName('GTiff').Create(output, cols, rows, 1, gdal.GDT_UInt16) 
...
def pre_process(settlment_file, landuse_file, crop_mask_file, output_shapefile):
	"""
	...
	if crop_mask_file != "None":
		domain = shp_file()
		domain.read(crop_mask_file)   # smaller selections. 
...
def cmd_line():
	...
	parser.add_argument('-c', '--config_file', type=str,  help='Configuration file name', required=True)
