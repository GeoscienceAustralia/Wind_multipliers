...
class GreatCircle(object):
    ...
    def points(self, npoints):
        ...
        # route is undefined.
        if self.antipodal:
            raise ValueError('cannot compute intermediate points on a great \
...
def vinc_dist(f, a, phi1, lembda1, phi2, lembda2):
    ...
    while (
        ...
        Cos_sigma = math.sin(U1) * math.sin(U2) + math.cos(U1) * math.cos(U2)\
            * math.cos(lembda)
        ...
        lembda = omega + (1 - C) * f * math.sin(alpha) * \
            (sigma + C * math.sin(sigma) * (Cos2sigma_m +
                                            C * math.cos(sigma) *
